...
# Current Features (이미 구현됨)

✅ **코어 기능:**
- 한국어 단어 3단계 난이도 퀴즈 (초급/중급/고급)
- 실시간 1:1 배틀 모드 (사용자 vs 사용자, 또는 사용자 vs AI)
- 난이도별 랭킹 시스템 및 TOP 10 순위표
- 사용자별 통계 (`/stats` 명령어)
- 자동 레벨 시스템 (연속 정답/오답에 따른 난이도 조정)
- 관리자 메뉴 (`/admin` 명령어)
  - 전체 통계 보기
  - 사용자 데이터 내보내기 (Excel, CSV)
  - 데이터베이스 백업
- GitHub 자동 백업 (선택 사항)
...
* 🌙 **학습 습관 트래킹**: 일일 학습 목표 및 연속 학습 일수 추적
* 📈 **학습 통계 대시보드**: 시간별/주별 학습량 분석 및 시각화
* 🤖 **AI 기반 동적 문제 생성**: LLM(ChatGPT/Claude)을 활용한 맞춤형 문제 생성
* 🎯 **개인 학습 경로**: 사용자 실력에 맞는 문제 추천 알고리즘
* 🏆 **시즌 랭킹**: 주간/월간 랭킹 초기화 및 시즌 보상
* 🌍 **다국어 지원**: 영어, 러시아어, 우즈벡어 등 다국어 인터페이스
* 📱 **모바일 웹 앱**: Telegram 외 웹 기반 학습 플랫폼
* 🎓 **학급 관리 기능**: 교사가 학생들의 진행도를 모니터링
* 🔊 **음성 발음 학습**: TTS(Text-to-Speech) 한국어 발음 제공
